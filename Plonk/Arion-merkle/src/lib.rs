...
use arion::sponge::merkle::hash as arion_merkle_hash;
...
/// A type that wraps a piece of data `T` together with a arion hash - i.e. a
/// [`BlsScalar`].
///
/// It implements [`Aggregate`] for any `T` that also implements the trait,
/// allowing for the construction of a arion tree without the need to define
/// where the aggregation of hashes is predefined.
///
/// # Example
/// ```rust
/// use dusk_bls12_381::BlsScalar;
/// use arion_merkle::{Item, Tree as ArionTree};
/// use arion::sponge;
/// use dusk_merkle::Aggregate;
///
    ...
    T: Aggregate<A>,
{
    ...
    fn aggregate(items: [&Self; A]) -> Self {
        ...
        // create new aggregated item with the hash being the arion hash of
        // the previous level
        Item {
            hash: arion_merkle_hash(&level_hashes),
